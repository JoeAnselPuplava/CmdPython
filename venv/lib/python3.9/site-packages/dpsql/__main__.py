...
def is_aggregate_function(node):
    # Check if the node represents an aggregate function
    if isinstance(node, ast.FuncCall):
        # List of aggregate function names (add more as needed)
        aggregate_functions = {'count', 'sum', 'avg', 'min', 'max'}
        if node.funcname and node.funcname[0].val.lower() in aggregate_functions:
            return True
    return False

def contains_aggregate_function(query):
    # Traverse the AST to check for aggregate functions
    root = parse_sql(query)
    # raw = Node(root)
    stmt = root[0]
    pprint(stmt(skip_none=True))
    for node in parse_sql(query):
        if isinstance(node, ast.RawStmt):
            stmt = node.stmt
            for subnode in stmt.traverse():
                if is_aggregate_function(subnode):
                    return True
    return False
