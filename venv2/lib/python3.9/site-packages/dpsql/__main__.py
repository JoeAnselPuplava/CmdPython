...
def is_aggregate_query(query):
    ...
    aggregates = ['sum', 'mean', 'max', 'min', 'std', 'var']
    return any(agg in query for agg in aggregates)

def apply_differential_privacy(data, query, epsilon):
    if not is_aggregate_query(query):
        raise ValueError("The provided query is not recognized as an aggregate query.")

    # Dynamically evaluate the query
    # Note: Use eval with caution, ensure the input is sanitized in production code
    result = eval(query)

    # Apply Laplace noise for differential privacy
    scale = 1.0 / epsilon
    base_laplace = make_base_laplace(scale=scale)
    noisy_result = base_laplace(result)
